...
%
% K-B-Td Parameter Space Sweep for EXP3 Stage 1
%
% Structure and classification logic kept consistent with exp2_sweep_final:
%   - Classification based purely on maxAbsX, maxAbsF, and envRatioX
%     (rmsErr thresholds removed; rmsErr is logged but not used in classification)
%   - Simulation horizon: 12 s (Tsim >= 2*W = 6 s → proper envelope windows)
%   - minWindowSec: 2.0 s (same as exp2_sweep_final)
%
% Speed strategy:
%   1) Short pre-check with its own small timeout
%   2) Only points classified decaying in pre-check go to full simulation
%   3) Each delay slice only tests points stable at the PREVIOUS delay slice
%   4) Timeout / solver failures are safely classified as divergent
...
cfg.modelName = 'exp3_bilateral_delay_stg1';
...
% early/late envelope windows are used (no quarter-sample fallback).
cfg.stopTimeFull      = '12';
cfg.stopTimePre       = '2.0';
cfg.simTimeoutSecFull = 120;
cfg.simTimeoutSecPre  = 15;
...
cfg.validationFolder    = 'exp3_validation_figures_stg1_clean';
...
% --- Stage 2 discrete params (harmless if unused by Stage 1 model) ---
cfg.Ts = 0.001;
cfg.a  = 0.92;

% --- Signal names (Stage 1) ---
cfg.signalNames.xM  = 'x_M';
cfg.signalNames.xS  = 'x_S';
cfg.signalNames.fcM = 'F_c_M';
cfg.signalNames.fcS = 'F_c_S';
...
cfg.figureFolder    = 'exp3_paramspace_figures_stg1';
cfg.resultFile      = 'exp3_paramspace_results_stg1.mat';
...
% stabilityMap(ki, bi, tdi): 0=decaying 1=near-critical 2=divergent 3=failed/timeout
stabilityMap = 3 * ones(nK, nB, nTd);
...
fprintf('=== EXP3 Stage 1 K-B-Td Parameter Space Sweep ===\n');
...
for ki = 1:nK
    for bi = 1:nB
        ...
        stabilityMap(ki, bi, 1) = res.classCode;
...
stableAtZero = (stabilityMap(:,:,1) == 0);
nStableBase  = sum(stableAtZero(:));

fprintf('\nPhase 1 complete in %.1f min.\n', toc(tPhase1)/60);
fprintf('%d / %d pairs decaying at Td=0.\n', nStableBase, nK*nB);
fprintf('Phase 2 will test only points decaying at the PREVIOUS delay slice.\n\n');

plotSlice(cfg, stabilityMap(:,:,1), 1, outDir);

if cfg.saveIncremental
    save(fullfile(scriptDir, cfg.resultFile), 'stabilityMap', 'cfg');
    fprintf('Phase 1 saved.\n\n');
end

%% =========================================================
%  OPTIONAL VALIDATION CANDIDATES
%% =========================================================
...
if cfg.autoValidate
    ...
    for ki = 1:nK
        for bi = 1:nB
            rows(end+1,:) = [cfg.KList(ki), cfg.BList(bi), stabilityMap(ki,bi,1)]; %#ok<AGROW>
        end
    end
...
%% =========================================================
%  PHASE 2: Delay sweep — only test pairs decaying at previous slice
%% =========================================================

fprintf('--- PHASE 2: Delay sweep ---\n');
tPhase2 = tic;

% Conservative upper bound only for progress reporting
phase2TotalRuns = nStableBase * (nTd - 1);
...
for tdi = 2:nTd
    ...
    stablePrev = (stabilityMap(:,:,tdi-1) == 0);

    % Pre-mark everything not decaying previously as divergent for this slice
    for ki = 1:nK
        for bi = 1:nB
            if ~stablePrev(ki, bi)
                stabilityMap(ki, bi, tdi) = 2;
            end
        end
    end

    % Collect only candidates decaying in previous slice
    pairsToTest = [];
    for ki = 1:nK
        for bi = 1:nB
            if stablePrev(ki, bi)
    ...
    if cfg.usePreCheck
        ...
        for p = 1:nPairsBeforePre
            ...
            if res.classCode ~= 0
                passedPre(p) = false;
                stabilityMap(ki, bi, tdi) = res.classCode;
            end
        end
    ...
    if nPairs == 0
        ...
        plotSlice(cfg, stabilityMap(:,:,tdi), tdi, outDir);
        if cfg.saveIncremental
            save(fullfile(scriptDir, cfg.resultFile), 'stabilityMap', 'cfg');
            fprintf('  Saved.\n');
        end
...
plotAllSlices(cfg, stabilityMap, outDir);
plotBoundaryEvolution(cfg, stabilityMap, outDir);
...
function runValidationSlice(cfg, valPts, Td_val, valDir)
    ...
    for p = 1:height(valPts)
        ...
        try
            ...
            sgtitle(fig, sprintf('EXP3 Stage 1 | %s | K=%.2f N/m  B=%.3f Ns/m  T_d=%.4f s', ...
...
function plotSlice(cfg, mapSlice, tdi, outDir)
    ...
    xlabel(ax, 'K [N/m]');
    ylabel(ax, 'B [Ns/m]');
    title(ax, sprintf('EXP3 Stage 1 — Response Map  |  T_d = %.4f s', Td_val), 'FontSize', 12);
...
function plotAllSlices(cfg, stabilityMap, outDir)
    if ~cfg.saveFigures; return; end
    ...
    sgtitle('EXP3 Stage 1 — Response Maps across Delays', 'FontSize', 13);
    exportgraphics(fig, fullfile(outDir, 'response_all_slices.png'), 'Resolution', 150);
    close(fig);
end

function plotBoundaryEvolution(cfg, stabilityMap, outDir)
    if ~cfg.saveFigures; return; end
    nTd    = numel(cfg.delayList);
    colors = parula(nTd);
    ...
    hold(ax,'on');
    for tdi = 1:nTd
        decayingCount = sum(stabilityMap(:,:,tdi) == 0, 2);
